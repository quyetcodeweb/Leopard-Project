...
const Taodonhang = ({ onClose, onCreated }) => {
    ...
    const fetchSanPham = async () => {
      try {
        ...
        const products = res.data.map((sp) => ({
          ProductID: sp.ProductID,          // bắt buộc có
          ten: sp.ProductName,              // hiển thị tên sản phẩm
          gia: sp.Price,                    // giá sản phẩm
  ...
  // ----------------------------
  // 2️⃣ Cập nhật thông tin khách hàng
  // ----------------------------
  ...
  const handleTao = async () => {
    ...
    const data = {
      ...
      sanPham: sanPhamHopLe.map((sp) => ({
        ProductID: sp.ProductID,  // chắc chắn có
        ten: sp.ten,
        soluong: sp.soluong,
        dongia: sp.gia,
      })),
    ...
    console.log("Dữ liệu gửi lên backend:", JSON.stringify(data, null, 2));
        ...
        <div className="row justify-content-center">
          ...
          <div className="d-flex flex-column" ref={leftColumnRef} style={{ width: "48%", marginRight: "2%" }}>
            <div className="p-3 mb-3 rounded" style={{ backgroundColor: "#F5F5F5" }}>
              ...
              <input className="form-control mb-2" type="text" name="ten" placeholder="Nhập tên khách hàng" value={khachHang.ten} onChange={handleKhachHangChange} />
              <input className="form-control mb-2" type="text" name="sdt" placeholder="Nhập số điện thoại" value={khachHang.sdt} onChange={handleKhachHangChange} />
              <input className="form-control mb-2" type="text" name="diachi" placeholder="Nhập địa chỉ khách hàng" value={khachHang.diachi} onChange={handleKhachHangChange} />
              <input className="form-control mb-2" type="text" name="maGiamGia" placeholder="Nhập mã giảm giá" value={khachHang.maGiamGia} onChange={handleKhachHangChange} />
            </div>
