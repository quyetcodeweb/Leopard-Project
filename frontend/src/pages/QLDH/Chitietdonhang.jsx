...
const Chitietdonhang = ({ orderID, onClose }) => {
  ...
  // Xác nhận đơn hàng => chuyển sang "Đã giao" và reload
  const handleConfirm = async () => {
    try {
      await axios.put(`http://localhost:5000/api/donhang/${donHang.maDonHang}/complete`);
      alert("Đơn hàng đã giao");
      window.location.reload(); // reload để cập nhật dữ liệu từ server
    } catch (err) {
      console.error(err);
    ...
    >
      ...
      >
        ...
        <div className="row mb-4" style={{ fontSize: "1rem" }}>
          <div className="col-8">
            <div className="mb-1"><strong>Khách hàng:</strong> {donHang.khachHang.ten}</div>
            <div className="mb-1"><strong>Số điện thoại:</strong> {donHang.khachHang.sdt}</div>
            <div className="mb-1"><strong>Địa chỉ:</strong> {donHang.khachHang.diachi}</div>
            <div className="mb-1"><strong>Ngày tạo:</strong> {new Date(donHang.ngayTao).toLocaleString("vi-VN", { hour12: false })}</div>
          </div>

          <div className="col-4 text-end">
            <div className="mb-2">
              <strong>Mã đơn hàng:</strong> <span className="fw-bold text-dark">{donHang.maDonHang}</span>
            </div>
            ...
            <div className="d-flex flex-column align-items-start mb-3">
              <div className="d-flex align-items-center mb-2">
                ...
                <input
                  ...
                  readOnly
                  style={{ width: 140, backgroundColor: "#f8f9fa", fontSize: "1rem", border: "1px solid #ced4da" }}
                />
              </div>
              ...
              {donHang.trangThai === "Đang xử lý" && (
                <div className="d-flex gap-2">
                  ...
                  <button className="btn btn-danger" onClick={handleCancel}>Hủy</button>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="d-flex justify-content-between align-items-center mt-3 pt-3" style={{ borderTop: "1px solid #dee2e6" }}>
          <h4 className="mb-0 fw-bold text-success">Tổng: {formatCurrency(donHang.tongTien)}</h4>
          <button className="btn btn-light" onClick={onClose} style={{ border: "1px solid #6C757D", color: "#6C757D" }}>
            Quay lại
          </button>
        </div>
      </div>
